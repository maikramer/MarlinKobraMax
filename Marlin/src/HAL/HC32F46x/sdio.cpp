...
bool SDIO_ReadBlock(uint32_t block, uint8_t *dst) {
  WITH_RETRY(SDIO_READ_RETRIES, {
    en_result_t rc =
        SDCARD_ReadBlocks(&cardHandle, block, 1, dst, SDIO_TIMEOUT);
    if (rc == Ok) {
      return true;
    } else {
...
bool SDIO_WriteBlock(uint32_t block, const uint8_t *src) {
  WITH_RETRY(SDIO_WRITE_RETRIES, {
    en_result_t rc =
        SDCARD_WriteBlocks(&cardHandle, block, 1, (uint8_t *)src, SDIO_TIMEOUT);
    if (rc == Ok) {
      return true;
    } else {
...
bool SDIO_IsReady() { return bool(cardHandle.stcCardStatus.READY_FOR_DATA); }

uint32_t SDIO_GetCardSize() {
  // multiply number of blocks with block size to get size in bytes
  uint64_t cardSizeBytes = uint64_t(cardHandle.stcSdCardInfo.u32LogBlockNbr) *
                           uint64_t(cardHandle.stcSdCardInfo.u32LogBlockSize);

  // if the card is bigger than ~4Gb (maximum a 32bit integer can hold), clamp
  // to the maximum value of a 32 bit integer
