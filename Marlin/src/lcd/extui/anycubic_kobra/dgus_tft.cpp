...
const uint8_t MESSAGE_bed_heating[] = {0xC8, 0xC8, 0xB4, 0xB2, 0xD5, 0xFD, 0xD4,
...
const uint8_t MESSAGE_ready[] = {0xD7, 0xBC, 0xB1, 0xB8, 0xBE,
                                 0xCD, 0xD0, 0xF7, 0x00};
const uint8_t MESSAGE_cold[] = {0xB4, 0xF2, 0xD3, 0xA1, 0xCD, 0xB7, 0xCE, 0xC2,
                                0xB6, 0xC8, 0xB9, 0xFD, 0xB5, 0xCD, 0x00};
...
DgusTFT::p_fun fun_array[] = {
    DgusTFT::page1_handle,  DgusTFT::page2_handle,  DgusTFT::page3_handle,
    DgusTFT::page4_handle,  DgusTFT::page5_handle,  DgusTFT::page6_handle,
    DgusTFT::page7_handle,  DgusTFT::page8_handle,  DgusTFT::page9_handle,
    DgusTFT::page10_handle, DgusTFT::page11_handle, DgusTFT::page12_handle,
    DgusTFT::page13_handle, DgusTFT::page14_handle, DgusTFT::page15_handle,
    DgusTFT::page16_handle, DgusTFT::page17_handle, DgusTFT::page18_handle,
    DgusTFT::page19_handle, DgusTFT::page20_handle, DgusTFT::page21_handle,
    DgusTFT::page22_handle, DgusTFT::page23_handle, DgusTFT::page24_handle,
    DgusTFT::page25_handle, DgusTFT::page26_handle, DgusTFT::page27_handle,
    DgusTFT::page28_handle, DgusTFT::page29_handle, DgusTFT::page30_handle,
    DgusTFT::page31_handle, DgusTFT::page32_handle, DgusTFT::page33_handle,
...
lcd_info_t
    DgusTFT::lcd_info_back; // back for changing on lcd, to save flash lifecycle
...
void DgusTFT::Startup() {
  ...
  // opt_set    FIL_RUNOUT_STATE HIGH  // Pin state indicating that filament is
  // NOT present. opt_enable FIL_RUNOUT_PULLUP
...
void DgusTFT::FilamentRunout() {
  ...
  //    SendtoTFTLN(isPrintingFromMedia() ? AC_msg_filament_out_alert :
  //    AC_msg_filament_out_block); printer_state = AC_printer_filament_out;
...
void DgusTFT::StatusChange(const char *const msg) {
  ...
  case AC_printer_probing: {
    ...
    if (strncmp(msg, MARLIN_msg_probing_point,
                strlen(MARLIN_msg_probing_point)) == 0) {
      probe_cnt++;
    }
...
void DgusTFT::PowerLossRecovery() {
  printer_state = AC_printer_resuming_from_power_outage; // Play tune to notify
                                                         // user we can recover.
}
...
void DgusTFT::SendtoTFT(
    PGM_P str) { // A helper to print PROGMEN string to the panel
#if ACDEBUG(AC_SOME)
  serialprintPGM(str);
#endif
  while (const uint8_t c = pgm_read_byte(str++))
    TFTSer.print(c);
}
...
void DgusTFT::page1_handle(void) {
  ...
  case 4: // system
    if (lcd_info.audio == ExtUI::ON) {
      ChangePageOfTFT(PAGE_SYSTEM_AUDIO_ON); // PAGE_SYSTEM_AUDIO_ON - 120
    } else if (lcd_info.audio == ExtUI::OFF) {
      ChangePageOfTFT(PAGE_SYSTEM_AUDIO_OFF); // PAGE_SYSTEM_AUDIO_OFF - 120
    }
    break;
  }
    ...
    if (lcd_txtbox_index > 0 && lcd_txtbox_index < 6) { // 1~5
      ...
      if (filenavigator.filelist.seek(lcd_txtbox_page * 5 +
        ...
        if (printer_state == AC_printer_resuming_from_power_outage) {
          ...
          //              SERIAL_ECHOLNPGM("start resumeing from power outage:
          //              ", AC_cmnd_power_loss_recovery);
...
void DgusTFT::page213_handle(void) { // MEL_MOD to display printer stats
  ...
  case 0x0999: // return
    if (lcd_info.audio == ExtUI::ON) {
      ...
      ChangePageOfTFT(50); // PAGE_SYSTEM_AUDIO_OFF - 120
    }
    // ChangePageOfTFT(PAGE_SYSTEM_AUDIO_ON);// go back to the ABOUT SOUND
    // enabled page
    break;
  }
}
